...
export default function KioskScrollSection({
  ...
  data = {},
  value,
  windowSize = 50, // 👈 number of frames to keep in memory
}) {
  const canvasRef = useRef(null);
  const imagesRef = useRef({});
  const animationRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Draw a frame if loaded
    const drawFrame = async (i) => {
      if (!imagesRef.current[i]) await loadFrame(i);
      const img = imagesRef.current[i];
      if (!img) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
    ...
    // Keep only frames around currentFrame in memory
    const cleanMemory = (currentFrame) => {
      for (const key in imagesRef.current) {
        if (Math.abs(key - currentFrame) > windowSize) {
          delete imagesRef.current[key];
        }
      }
    };

    // Initial canvas size from first frame
  ...
  }, [prefix, frameCount, folder, frameExt, value, windowSize]);
