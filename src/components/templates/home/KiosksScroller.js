...
export default function KioskScrollSection({
  prefix = "block2",
  frameCount = 300,               // ⚡ After frame reduction
  folder = "/assets/",
  frameExt = "webp",              // ⚡ WebP only
  data = {},
  value,
}) {
  const canvasRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // ⚡ Load first 30 frames immediately
    const INITIAL_LOAD = 30;

    for (let i = 1; i <= Math.min(frameCount, INITIAL_LOAD); i++) {
      images.push(loadFrame(i));
    }
    ...
    // ⚡ Background Idle loading for remaining frames
    let currentFrame = INITIAL_LOAD + 1;

    const idleLoader = (deadline) => {
      while (deadline.timeRemaining() > 0 && currentFrame <= frameCount) {
        imagesRef.current[currentFrame] = loadFrame(currentFrame);
        currentFrame++;
      }

      if (currentFrame <= frameCount) {
        requestIdleCallback(idleLoader);
      }
    };
